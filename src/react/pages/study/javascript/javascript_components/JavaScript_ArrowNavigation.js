...
const JavaScript_ArrowNavigation = ({ direction }) => {
  ...
  // 현재 챕터와 콘텐츠 찾기
  const currentChapter = JavaScriptStudyChapter.find(
    ...
    if (direction === "left") {
      if (currentContentIndex === 0) {
        ...
        const previousChapterIndex =
          JavaScriptStudyChapter.findIndex(
            (chapter) => chapter.id === currentChapterId
          ) - 1;
        const previousChapter = JavaScriptStudyChapter[previousChapterIndex];
      ...
      if (nextContentIndex >= currentChapter.contents.length) {
        ...
        const nextChapter = JavaScriptStudyChapter[nextChapterIndex];
...
export default JavaScript_ArrowNavigation;
