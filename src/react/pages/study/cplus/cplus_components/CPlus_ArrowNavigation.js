...
const CPlus_ArrowNavigation = ({ direction }) => {
  ...
  // 현재 챕터와 콘텐츠 찾기
  const currentChapter = CPlusStudyChapter.find(
    ...
    if (direction === "left") {
      if (currentContentIndex === 0) {
        ...
        const previousChapterIndex =
          CPlusStudyChapter.findIndex(
            (chapter) => chapter.id === currentChapterId
          ) - 1;
        const previousChapter = CPlusStudyChapter[previousChapterIndex];
      ...
      if (nextContentIndex >= currentChapter.contents.length) {
        ...
        const nextChapter = CPlusStudyChapter[nextChapterIndex];
...
export default CPlus_ArrowNavigation;
