      ...
      <TopBoxWide2>
        <TopBox2>
          ...
          <TopBoxLink onClick={() => handleRefresh()}>
            <TopBoxText2>02. 변수와 시스템 입출력</TopBoxText2>
          </TopBoxLink>
        </TopBox2>
      </TopBoxWide2>
      <Container>
        ...
        <ClassContentsText>
          사용자가 프로그램과 대화하기 위해서는 사용자와 프로그램 사이의
          입출력을 담당하는 수단이 필요 합니다.
          <br />
          자바에서는 모든 것이 객체로 표현되므로, 입출력을 담당하는 수단 또한
          모두 객체 입니다.
          <br />
          <br />
          <ClassContentsTextTab>
            ...
            표준 입력 스트림으로, 프로그램 내에서 사용자로부터의 입력을 받을 수
            있도록 합니다.
            <br />-{" "}
            <font color="#2c79c1">
              <i>System.out</i>
            </font>{" "}
            표준 출력 스트림으로, 프로그램 내에서 결과를 콘솔에 출력할 수 있도록
            합니다.
            <br />-{" "}
            <font color="#2c79c1">
              <i>System.err</i>
            </font>{" "}
            표준 오류 출력 스트림으로, 프로그램 내에서 발생한 오류와 관련된
            정보를 콘솔에 출력할 때 사용됩니다. (리다이렉션이 되지 않습니다.)
            <br />
            <br />
          </ClassContentsTextTab>
          <ClassContentsCodeBox>
            👉 리다이렉션이란 컴퓨터 프로그램이나 운영 체제에서 입력과 출력을
            다른 곳으로 보내는 기능을 말합니다.
          </ClassContentsCodeBox>
          <br />
        ...
        <ClassContentsText>
          ...
          하지만,{" "}
          <b>
            java.lang 패키지에 속하는 System 클래스를 이용하면 운영체제의 일부
            기능을 이용
          </b>
          할 수 있습니다.
          <br />
          즉 프로그램의 종료, 키보드 입력, 모니터 출력, 메모리 정리, 현재 시간
          읽기, 시스템 환경 변수 읽기 등이 가능 합니다.
          <br />
        ...
        <ClassContentsText>
          키보드에서 키 하나를 입력하면 프로그램에서는 숫자로 된 키보드를 읽을
          수 있습니다.
          <br />
          <font color="red">
          ...
          <br />
          자바는 일출력에 대한 예외에 대해서 까다롭게 규정하고 있습니다. 그래서
          입/출력 을 다루는 메서드에서 예외처리가 없다면 컴파일 에러가 발생하게
          됩니다.
          <br />
          <br />
          <font color="red">
        ...
        <ClassContentsText>
          1. 기본적인 데이터 타입들을 Scanner의 메소드를 사용하여 입력받을 수
          있습니다.
          <br />
          예로 들어 100을 입력하고자 할 때, String(문자열)로 입력받고 싶으면
          next() 나 nextLine() 을, int(정수)로 입력받고 싶다면 nextInt() 를
          사용하여 입력받으면 알아서 해당 타입으로 입력된다.
          <br />
          2. Scanner 을 사용할 시 util 패키지를 경로의 Scanner 클래스를 호출해야
          합니다.
          <br />
        ...
        <ClassContentsText>
          스캐너 클래스 사용을 위해서는 먼저 클래스를 호출하고, 객체를 생성 해야
          합니다.
          <br />
          <b>
            클래스이름 객체이름 = <font color="#2c79c1">new</font> 클래스이름();
          </b>
          <br />
          <font color="red">
            <b>Scanner in = new Scanner(System.in); // Scanner 객체 생성</b>
          </font>{" "}
          <b>[Scanner 입력 메소드 종류]</b>
          <br />
          Scanner 생성자에는 사용자로 부터 입력을 받기 위한 입력 스트림을 지정
          합니다.
        </ClassContentsText>
